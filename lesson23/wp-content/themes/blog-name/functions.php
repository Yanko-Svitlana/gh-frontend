<?php
/**
 * Blog Name functions and definitions
 *
...
function blog_name_setup() {
	 ...
	 * If you're building a theme based on Blog Name, use a find and replace
	 * to change 'blog-name' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'blog-name', get_template_directory() . '/languages' );
...
function blog_name_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'blog-name' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'blog-name' ),
...
function blog_name_scripts() {
	...
	wp_enqueue_style( 'blog-name-style', get_stylesheet_uri() );

	wp_enqueue_script( 'blog-name-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );
...
function custom_search_form( $form ) {
	...
	$form = '<form role="search" method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<ul>
					...
					<li>
						<input type="image" class="search-submit" src="'. get_template_directory_uri().'/img/search.png" />
					</li>
				</ul>
				</form>';
